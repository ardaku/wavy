...
pub struct Microphone<C: Channel + Unpin, F: Config + Unpin> {
    microphone: ffi::Microphone<C, F>,
}

impl<C: Channel + Unpin + From<Ch16>, F: Config + Unpin> Microphone<C, F> {
    /// Connect to a microphone.  Unlike `Speakers`, you may call this multiple
    /// times to connect to multiple devices.
    pub fn new() -> Option<Self> {
        Some(Self { microphone: ffi::Microphone::new()? })
    }
    ...
    pub async fn record(&mut self) -> impl StreamRecv<Sample1<C, F>> + '_ {
