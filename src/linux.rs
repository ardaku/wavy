...
fn pcm_open(microphone: bool, name: &[u8]) -> Result<*mut snd_pcm_t, AudioError> {
    ...
    if unsafe {
        // plughw:0,0 ?
        snd_pcm_open(&mut sound_device, name.as_ptr() as *const _, if microphone { 1 /* capture */ } else { 0 /*playback*/ }, 0)
    } < 0 {
...
fn pcm_hw_params(sr: SampleRate, sound_device: *mut snd_pcm_t) -> Result<*mut snd_pcm_hw_params_t, AudioError> {
    let mut hw_params: *mut snd_pcm_hw_params_t = unsafe { std::mem::uninitialized() };
    if unsafe {
        snd_pcm_hw_params_malloc(&mut hw_params)
    } < 0 {
        return Err(AudioError::InternalError("Cannot allocate hardware parameter structure!".to_string()));
    }
    if unsafe {
        snd_pcm_hw_params_any(sound_device, hw_params)
    } < 0 {
        return Err(AudioError::InternalError("Cannot initialize hardware parameter structure!".to_string()));
    }
    // Enable resampling.
    if unsafe {
        snd_pcm_hw_params_set_rate_resample(sound_device, hw_params, 1)
    } < 0 {
        return Err(AudioError::InternalError("Resampling setup failed for playback!".to_string()));
    }
    ...
    if unsafe {
        ...
        return Err(AudioError::InternalError("Cannot set sample format!".to_string()));
    }
    // Set channels to stereo (2).
    if unsafe {
        snd_pcm_hw_params_set_channels(sound_device, hw_params, 2)
    } < 0 {
        return Err(AudioError::InternalError("Cannot set channel count!".to_string()));
    }
    // Set Sample rate.
    ...
    if unsafe {
        ...
        return Err(AudioError::InternalError("Cannot set sample rate!".to_string()));
    }
    if actual_rate != sr as u32 {
        return Err(AudioError::InternalError(format!("Failed to set rate: {}, Got: {} instead!", sr as u32, actual_rate)));
    }
    // Apply the hardware parameters that just got set.
    if unsafe {
        snd_pcm_hw_params(sound_device, hw_params)
    } < 0 {
        return Err(AudioError::InternalError("Failed to set parameters!".to_string()));
    }
